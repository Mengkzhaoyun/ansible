...
if [[ -e /etc/kubernetes/ssl/kubeproxy.pem ]]; then
  ...
  openssl x509 -req -in kubeproxy.csr -CA ca.pem -CAkey ca.key -CAcreateserial -out kubeproxy.pem -days 3650 -extensions v3_req -extfile worker-kubeproxy.cnf
  echo 'kubeproxy.pem is ready!'
fi
...
if [[ -e /etc/kubernetes/ssl/kubelet.pem ]]; then
  ...
  openssl x509 -req -in kubelet.csr -CA ca.pem -CAkey ca.key -CAcreateserial -out kubelet.pem -days 3650 -extensions v3_req -extfile worker-kubelet.cnf  
  echo 'kubelet.pem is ready!'
fi